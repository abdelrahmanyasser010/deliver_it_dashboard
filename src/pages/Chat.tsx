...
export function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    if (!text) return;
    const result = await execute({ type: 'chat/send', roomId: activeRoom.id, text, note: noteMode, actor: 'موظف خدمة العملاء' });
    showToast(result.message, result.ok ? 'success' : 'danger');
    if (result.ok) { setDraft(''); setNoteMode(false); }
  };
  ...
  return <div className="chat-page chat-v2">
    ...
    <section className="chat-window glass-card">
      ...
      <div className="chat-messages" aria-live="polite">{activeRoom.messages.map((message) => <div key={message.id} className={`chat-message ${message.type}`}><span>{message.type === 'note' && <StickyNote size={13}/>} {message.text}</span><small>{message.time}{message.type === 'note' ? ' · ملاحظة داخلية' : ''}</small></div>)}</div>
      <div className={`chat-compose ${noteMode ? 'note-mode' : ''}`}><div className="chat-tools"><button aria-label="إرفاق ملف" onClick={() => showToast('المرفقات ستُرفع عبر Media API عند ربط الـBackend.', 'info')}><Paperclip size={16}/></button><button aria-label="إضافة الموقع" onClick={() => setDraft((value) => `${value}${value ? ' ' : ''}📍 رابط الموقع`)}><MapPin size={16}/></button><button className={noteMode ? 'active' : ''} aria-label="ملاحظة داخلية" onClick={() => setNoteMode((value) => !value)}><StickyNote size={16}/></button></div><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) void sendMessage(); }} placeholder={noteMode ? 'اكتب ملاحظة داخلية لا يراها الطرف الآخر...' : 'اكتب ردًا...'} aria-label={noteMode ? 'ملاحظة داخلية' : 'رسالة'} disabled={activeRoom.status === 'closed'} /><button className="btn-primary" onClick={() => void sendMessage()} disabled={!draft.trim() || activeRoom.status === 'closed'}><Send size={16}/> إرسال</button></div>
    </section>

    <aside className="chat-context glass-card"><h3>السياق المرتبط</h3>{linkedShipment ? <><div className="chat-shipment-card"><div><span>الشحنة</span><strong>{linkedShipment.id}</strong></div><StatusBadge label={statusConfig[linkedShipment.status].label} tone={linkedShipment.status === 'delivered' ? 'success' : linkedShipment.status === 'returned' ? 'danger' : 'info'}/><p>{linkedShipment.customerName}</p><p dir="ltr">{linkedShipment.customerPhone}</p><dl><div><dt>التاجر</dt><dd>{linkedShipment.merchantName}</dd></div><div><dt>المندوب</dt><dd>{linkedShipment.driverName ?? 'غير معين'}</dd></div><div><dt>التحصيل</dt><dd>{formatCurrency(linkedShipment.expectedCollection)}</dd></div><div><dt>المحاولات</dt><dd>{linkedShipment.attemptCount.toLocaleString('ar-EG')}</dd></div></dl><button className="btn-primary" onClick={() => navigate(`/shipments?shipment=${linkedShipment.id}`)}><PackageSearch size={15}/> فتح الشحنة</button></div><div className="chat-assignment"><UserRoundCheck size={17}/><section><strong>المسؤول الحالي</strong><small>{activeRoom.assignedTo}</small></section><button onClick={() => { setTransferTo(activeRoom.assignedTo); setTransferOpen(true); }}>تحويل</button></div></> : <EmptyState title="لا يوجد كيان مرتبط" description="يمكن للـBackend لاحقًا ربط المحادثة بشحنة أو تاجر أو تذكرة."/>}</aside>
