import { CheckCircle2, MessageCircle, PackageSearch, Paperclip, Pin, Search, Send, StickyNote, UserRoundCheck, X } from 'lucide-react';
...
type StaffUser = { id: string; name: string; email?: string | null; status?: string; membership_status?: string };
...
export function ChatPage() {
  ...
  useEffect(() => {
    ...
    void api.get<StaffUser[]>('/api/v1/staff-users', { query: { status: 'active', page: 1, per_page: 100 } })
      .then((result) => {
        if (!active) return;
        const users = (result.data ?? []).filter((user) => user.status !== 'archived' && user.membership_status !== 'archived');
        setStaffUsers(users);
        const currentId = activeRoom?.assignedUserId;
        setTransferTo((value) => value && users.some((user) => user.id === value) ? value : currentId && users.some((user) => user.id === currentId) ? currentId : users[0]?.id ?? '');
      })
      .catch((error) => { if (active) showToast(friendlyApiMessage(error), 'danger'); })
      .finally(() => { if (active) setStaffLoading(false); });
    return () => { active = false; };
  }, [transferOpen, activeRoom?.assignedUserId, showToast]);
  ...
  const openRoom = async (id: string) => {
    ...
    await execute({ type: 'chat/read', roomId: id });
  };

  const sendMessage = async () => {
    if (!activeRoom || uploading) return;
    const text = draft.trim();
    if (!text && attachments.length === 0) return;
    const result = await execute({ type: 'chat/send', roomId: activeRoom.id, text, note: noteMode, attachments });
  ...
  const addAttachments = async (files: FileList | null) => {
    ...
    if (!available) { showToast('الحد الأقصى 10 مرفقات لكل رسالة.', 'warning'); return; }
    setUploading(true);
    const uploaded: ChatAttachmentRecord[] = [];
    try {
      for (const file of selected) {
        const completed = await uploadApiFile(file, 'chat_attachment');
        uploaded.push({ id: completed.id, name: completed.file_name, mimeType: completed.mime_type, size: completed.size_bytes, url: completed.download_url ?? undefined });
      }
      setAttachments((current) => [...current, ...uploaded].slice(0, 10));
      if (uploaded.length) showToast(`تم رفع ${uploaded.length.toLocaleString('ar-EG')} مرفق وجاهز للإرسال.`, 'success');
    } catch (error) {
      showToast(friendlyApiMessage(error), 'danger');
    } finally {
      setUploading(false);
    }
  };
  ...
  const toggleClosed = async () => {
    ...
    const result = await execute({ type: 'chat/toggle', roomId: activeRoom.id });
    showToast(result.message, result.ok ? 'info' : 'danger');
  };

  const transfer = async () => {
    if (!activeRoom || !transferTo) return;
    const result = await execute({ type: 'chat/transfer', roomId: activeRoom.id, assignedTo: transferTo });
    showToast(result.message, result.ok ? 'success' : 'danger');
  ...
  return <div className="chat-page chat-v2">
    ...
    <section className="chat-window glass-card">
      ...
      <input ref={fileInputRef} type="file" hidden multiple accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx,.xls,.xlsx,.csv" onChange={(event) => { void addAttachments(event.target.files); event.currentTarget.value = ''; }} />
      <div className={`chat-compose ${noteMode ? 'note-mode' : ''}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void addAttachments(event.dataTransfer.files); }}><div className="chat-tools"><button aria-label="إرفاق صورة أو ملف" onClick={() => fileInputRef.current?.click()} disabled={uploading}><Paperclip size={16}/></button><button className={noteMode ? 'active' : ''} aria-label="ملاحظة داخلية" onClick={() => setNoteMode((value) => !value)}><StickyNote size={16}/></button></div><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) void sendMessage(); }} placeholder={uploading ? 'جارٍ رفع المرفقات...' : noteMode ? 'اكتب ملاحظة داخلية لا يراها الطرف الآخر...' : 'اكتب ردًا أو اسحب ملفًا هنا...'} aria-label={noteMode ? 'ملاحظة داخلية' : 'رسالة'} disabled={activeRoom.status === 'closed' || uploading} /><button className="btn-primary" onClick={() => void sendMessage()} disabled={uploading || (!draft.trim() && attachments.length === 0) || activeRoom.status === 'closed'}><Send size={16}/> {uploading ? 'يرفع...' : 'إرسال'}</button></div>
    </section>

    <aside className="chat-context glass-card"><h3>السياق المرتبط</h3>{linkedShipment ? <><div className="chat-shipment-card"><div><span>الشحنة</span><strong>{linkedShipment.id}</strong></div><StatusBadge label={statusConfig[linkedShipment.status].label} tone={['delivered', 'partiallyDelivered'].includes(linkedShipment.status) ? 'success' : linkedShipment.status === 'returned' ? 'danger' : 'info'}/><p>{linkedShipment.customerName}</p><p dir="ltr">{linkedShipment.customerPhone}</p><dl><div><dt>التاجر</dt><dd>{linkedShipment.merchantName}</dd></div><div><dt>المندوب</dt><dd>{linkedShipment.driverName ?? 'غير معين'}</dd></div><div><dt>التحصيل</dt><dd>{formatCurrency(linkedShipment.expectedCollection)}</dd></div><div><dt>المحاولات</dt><dd>{linkedShipment.attemptCount.toLocaleString('ar-EG')}</dd></div></dl><button className="btn-primary" onClick={() => navigate(`/shipments?shipment=${linkedShipment.id}`)}><PackageSearch size={15}/> فتح الشحنة</button></div><div className="chat-assignment"><UserRoundCheck size={17}/><section><strong>المسؤول الحالي</strong><small>{activeRoom.assignedTo}</small></section><button onClick={() => setTransferOpen(true)}>تحويل</button></div></> : <EmptyState title="لا يوجد كيان مرتبط" description="لا يوجد كيان تشغيلي مرتبط بهذه المحادثة حاليًا."/>}</aside>

    {transferOpen && <Modal title="تحويل المحادثة" description="اختر موظفًا فعالًا من نفس الشركة. يسجل الخادم عملية التحويل في سجل التدقيق." onClose={() => setTransferOpen(false)} footer={<><button className="outline-btn" onClick={() => setTransferOpen(false)}>إلغاء</button><button className="btn-primary" disabled={!transferTo || staffLoading} onClick={() => void transfer()}>تأكيد التحويل</button></>}><label className="form-field"><span>المسؤول الجديد</span><select className="input-glass" value={transferTo} disabled={staffLoading} onChange={(event) => setTransferTo(event.target.value)}><option value="">{staffLoading ? 'جارٍ تحميل الموظفين...' : 'اختر موظفًا'}</option>{staffUsers.map((staff) => <option key={staff.id} value={staff.id}>{staff.name}{staff.email ? ` — ${staff.email}` : ''}</option>)}</select></label></Modal>}
  </div>;
}
