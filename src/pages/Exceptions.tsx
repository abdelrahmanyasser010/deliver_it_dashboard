...
const categoryLabels: Record<ExceptionCategory, string> = { all: 'كل الحالات', delay: 'تأخير', assignment: 'بلا مندوب', financial: 'فروقات مالية', return: 'مرتجعات', customer: 'خدمة العملاء', stale: 'بدون تحديث' };
...
export function ExceptionsPage() {
  ...
  return <div className="exceptions-page"><SectionHeader title="مركز المتابعة" description="معالجة الحالة ينفذ الإجراء الحقيقي أو يسجل سبب الإغلاق في سجل الشحنة." actions={<button className="outline-btn" onClick={refetch}><RefreshCcw size={15}/> تحديث</button>}/><div className="exceptions-metrics"><MetricCard label="إجمالي المشكلات" value={exceptions.length.toLocaleString('ar-EG')} detail="مشكلات الشحنات المطلوبة معالجتها" icon={AlertTriangle} tone="danger"/><MetricCard label="تأخير" value={Number(counts.delay).toLocaleString('ar-EG')} detail="تجاوزت الموعد" icon={Clock3} tone="warning" onClick={() => setCategory('delay')}/><MetricCard label="مشكلات مالية" value={Number(counts.financial).toLocaleString('ar-EG')} detail="فرق تحصيل أو مراجعة" icon={Banknote} tone="danger" onClick={() => setCategory('financial')}/><MetricCard label="بلا مندوب" value={Number(counts.assignment).toLocaleString('ar-EG')} detail="تحتاج توزيعًا" icon={UserRoundX} tone="warning" onClick={() => setCategory('assignment')}/></div><section className="exceptions-toolbar glass-card"><input className="input-glass" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث في الحالات..." aria-label="البحث في الحالات"/><div className="exceptions-tabs">{(Object.keys(categoryLabels) as ExceptionCategory[]).map((category) => <button key={category} className={selectedCategory === category ? 'active' : ''} onClick={() => setCategory(category)}>{categoryLabels[category]} <span>{Number(counts[category]).toLocaleString('ar-EG')}</span></button>)}</div></section>{filtered.length === 0 ? <EmptyState title="لا توجد حالات مطابقة" description="تمت المعالجة أو لا توجد نتائج للفلاتر الحالية." actionLabel="عرض الكل" onAction={() => { setCategory('all'); setQuery(''); }}/> : <section className="exceptions-list glass-card">{filtered.map((item) => <article key={item.id} className={`exception-row ${item.severity}`}><span className="exception-symbol">{exceptionIcon(item.category)}</span><div className="exception-main"><div className="exception-title-line"><strong>{item.title}</strong><StatusBadge label={severityLabel(item.severity)} tone={item.severity === 'urgent' ? 'danger' : item.severity === 'high' ? 'warning' : 'info'}/></div><p>{item.reason}</p><div className="exception-meta"><span>{item.shipment.id}</span><span>{item.shipment.customerName}</span><span>{item.shipment.merchantName}</span><span>{item.shipment.driverName ?? 'غير معين'}</span><span>{formatCurrency(item.shipment.expectedCollection)}</span><span>{formatAge(item.shipment.lastUpdatedAt)}</span></div></div><div className="exception-actions"><button className="outline-btn" onClick={() => navigate(`/shipments?shipment=${item.shipment.id}`)}><PackageSearch size={15}/> فتح الشحنة</button><button className="btn-primary" onClick={() => openResolve(item)}>معالجة الحالة</button></div></article>)}</section>}
  {resolutionItem && <Modal title={`معالجة: ${resolutionItem.title}`} description={`${resolutionItem.shipment.id} — ${resolutionItem.reason}`} onClose={() => setResolutionItem(null)} footer={<><button className="outline-btn" onClick={() => setResolutionItem(null)}>إلغاء</button><button className="btn-primary" onClick={() => void resolve()}>{resolutionItem.category === 'financial' ? 'فتح المطابقة المالية' : 'تأكيد المعالجة'}</button></>}>{resolutionItem.category === 'assignment' && <label className="form-field"><span>المندوب</span><select className="input-glass" value={driverId} onChange={(event) => setDriverId(event.target.value)}>{drivers.filter((driver) => driver.status === 'active').map((driver) => <option key={driver.id} value={driver.id}>{driver.name} — متاح {Math.max(0, driver.capacity - driver.activeLoad).toLocaleString('ar-EG')}</option>)}</select></label>}{resolutionItem.category !== 'financial' && <label className="form-field"><span>تفاصيل المعالجة</span><textarea className="input-glass" value={resolution} onChange={(event) => setResolution(event.target.value)}/></label>}{resolutionItem.category === 'financial' && <p className="report-muted">سيتم فتح شاشة المطابقة المالية على الشحنة، لأن إخفاء الحالة دون تعديل المبالغ غير مسموح.</p>}</Modal>}</div>;
}
