...
import { Barcode, CheckCircle2, ClipboardCheck, PackageCheck, Route, Search, Truck, Undo2, XCircle } from 'lucide-react';
...
export function OperationsCenterPage() {
  ...
  const returns = shipments.filter((shipment) => shipment.taskStatus === 'needsReturnProcessing');
  ...
  return <div className="operations-center">
    ...
    {activeTab === 'driverUpdates' && <section className="ops-section compact-flow">{pendingUpdates.filter((update) => matches([update.id, update.shipmentId, update.driverName, update.merchantName])).map((update) => <article key={update.id} className="ops-row-card glass-card"><div><div className="ops-title-line"><strong>{update.shipmentId}</strong><StatusBadge label={update.reportedStatus} tone="warning"/></div><p className="ops-meta">{update.driverName} · {update.merchantName} · {update.customerName}</p><small className="ops-meta">{update.note ?? update.evidence}</small></div><div><span className="ops-badge info">الحالة السابقة: {update.previousStatus}</span></div><div className="ops-row-actions"><button className="outline-btn" onClick={() => void run({ type: 'driverUpdate/reject', updateId: update.id })}><XCircle size={15}/> رفض</button><button className="btn-primary" onClick={() => void run({ type: 'driverUpdate/approve', updateId: update.id })}><CheckCircle2 size={15}/> اعتماد وتحديث الشحنة</button></div></article>)}{pendingUpdates.length === 0 && <EmptyState title="لا توجد تحديثات معلقة" description="كل تحديثات المناديب تم اعتمادها أو مراجعتها."/>}</section>}

    {activeTab === 'returns' && <section className="ops-section">{returns.filter((shipment) => matches([shipment.id, shipment.merchantName, shipment.driverName])).map((shipment) => <ShipmentTaskRow key={shipment.id} shipment={shipment} action={<button className="btn-primary" onClick={() => void run({ type: 'exception/resolve', shipmentId: shipment.id, resolution: 'تم تسليم المرتجع للتاجر وإغلاق المهمة.' })}>تأكيد التسليم للتاجر</button>}/>)}{returns.length === 0 && <EmptyState title="لا توجد مرتجعات معلقة" description="دورة المرتجعات محدثة."/>}</section>}
  </div>;
}
