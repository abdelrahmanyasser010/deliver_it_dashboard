...
const pickupRequestsSeed: PickupRequest[] = [
  {
    ...
    phone: '01011223344',
    shipments: ['SHP-1001', 'SHP-1002', 'SHP-1003', 'SHP-1004', 'SHP-1005'],
    codTotal: 4850,
    drivers: ['أحمد سامي', 'محمد علي'],
    priority: 'high',
  },
  {
    id: 'PKG-1042',
    ...
    address: 'شارع العريش',
    phone: '01122334455',
    shipments: ['SHP-1010', 'SHP-1011', 'SHP-1012'],
    codTotal: 3120,
    drivers: ['خالد إبراهيم', 'ياسر عمر'],
    priority: 'normal',
  },
  {
    id: 'PKG-1043',
    ...
    phone: '01299887766',
    shipments: ['SHP-1020', 'SHP-1021', 'SHP-1022', 'SHP-1023'],
    codTotal: 12900,
    drivers: ['ياسر عمر', 'محمد علي'],
    priority: 'high',
  },
];
...
const updatesSeed: DriverUpdate[] = [
  {
    ...
    fromStatus: 'مع المندوب',
    toStatus: 'تم التسليم',
    note: 'تم التحصيل بالكامل وإثبات التسليم موجود',
    risk: 'low',
    decision: 'pending',
  },
  {
    ...
    merchantName: 'هوم ديكور',
    customerName: 'نهى سالم',
    fromStatus: 'في الطريق',
    toStatus: 'فشل التسليم',
    note: 'العميل لا يرد بعد محاولتين',
    risk: 'medium',
    decision: 'pending',
  },
  {
    ...
    fromStatus: 'مع المندوب',
    toStatus: 'مرتجع',
    note: 'العميل رفض الاستلام بسبب المقاس',
    risk: 'high',
    decision: 'pending',
  },
];
...
export function OperationsCenterPage() {
  ...
  const [activityMessage, setActivityMessage] = useState<string | null>('ابدأ من أعلى أولوية: استلامات التجار، ثم تأكيد الوصول، ثم إسناد التوصيل.');
  ...
  const visibleBoxes = useMemo(() => {
    return boxes.map((box) => ({
      ...box,
      shipments: box.shipmentIds
        .map((id) => deliveryShipments.find((shipment) => shipment.id === id))
        .filter(Boolean) as DeliveryShipment[],
    }));
  }, [boxes]);
  ...
  const regenerateBoxes = (nextCapacity: number) => {
    ...
    setActivityMessage(`تم إعادة ترتيب مجموعات التوصيل بسعة ${nextCapacity} شحنات لكل مندوب.`);
  };
  ...
  const dispatchPickup = (requestId: string, driverName: string, shipmentIds: string[]) => {
    ...
    if (!request || shipmentIds.length === 0) {
      setActivityMessage('حدد شحنة واحدة على الأقل قبل إرسال المندوب.');
      return;
    }

    const amountPerShipment = Math.round(request.codTotal / request.shipments.length);
    const newReviewLines = shipmentIds.map((shipmentId) => ({
      id: shipmentId,
      merchantName: request.merchantName,
      driverName,
      status: 'waiting' as PickupLineStatus,
      codAmount: amountPerShipment,
    }));

    setReviewLines((lines) => [...newReviewLines, ...lines.filter((line) => !shipmentIds.includes(line.id))]);
    setPickupRequests((requests) => requests
      .map((item) => {
        ...
        return {
          ...
          codTotal: amountPerShipment * remainingShipments.length,
        };
      })
      .filter((item) => item.shipments.length > 0));
    setActivityMessage(`تم إرسال ${shipmentIds.length} شحنات من ${request.merchantName} إلى ${driverName}، وظهرت الآن في تأكيد الاستلام.`);
  };
  ...
  const scanShipment = () => {
    ...
    setReviewLines((lines) => lines.map((line) => (
      line.id.toUpperCase() === normalized ? { ...line, status: 'verified' } : line
    )));
  ...
  const markReviewLine = (shipmentId: string, status: PickupLineStatus) => {
    setReviewLines((lines) => lines.map((line) => (
      line.id === shipmentId ? { ...line, status } : line
    )));
  ...
  const approveReviewed = () => {
    ...
    if (approved === 0) {
      setActivityMessage('لا توجد شحنات معلّمة كمستلمة لاعتمادها.');
      return;
    }

    setReviewLines((lines) => lines.filter((line) => line.status !== 'verified'));
    setActivityMessage(`تم اعتماد ${approved} شحنات وإظهار نتيجة الاستلام للتاجر.`);
  };

  const chooseBoxDriver = (boxId: string, driverId: string) => {
    setBoxes((currentBoxes) => currentBoxes.map((box) => (
      box.id === boxId ? { ...box, selectedDriverId: driverId } : box
    )));
  };
  ...
  const sendBox = (boxId: string) => {
    ...
    setBoxes((currentBoxes) => currentBoxes.map((item) => (
      item.id === boxId ? { ...item, sent: true } : item
    )));
    setActivityMessage(`تم إسناد ${box.shipmentIds.length} شحنات إلى ${driver.name}.`);
  };
  ...
  const decideUpdate = (updateId: string, decision: DriverDecision) => {
    ...
    setUpdates((currentUpdates) => currentUpdates.map((item) => (
      item.id === updateId ? { ...item, decision } : item
    )));
  ...
  const approveLowRiskUpdates = () => {
    ...
    setUpdates((currentUpdates) => currentUpdates.map((update) => (
      update.risk === 'low' && update.decision === 'pending' ? { ...update, decision: 'approved' } : update
    )));
    setActivityMessage(`تم اعتماد ${lowRiskCount} تحديثات منخفضة المخاطر.`);
  };
  ...
  return (
    <div className="operations-center">
      <header className="ops-command-bar glass-card">
        <div>
          <h2>مركز العمليات</h2>
          <p>شاشة قرار مختصرة: ابعت المندوب، أكد الاستلام، وزع التوصيل، واعتمد تحديثات اليوم.</p>
        </div>
      ...
      {activeTab === 'pickupRequests' && (
        <PickupRequestsTab requests={pickupRequests} onDispatch={dispatchPickup} />
      )}

      {activeTab === 'pickupReview' && (
        <PickupReviewTab
          reviewLines={reviewLines}
          scanValue={scanValue}
          onApproveReviewed={approveReviewed}
          onMarkReviewLine={markReviewLine}
          onScan={scanShipment}
      ...
      {activeTab === 'assignment' && (
        <AssignmentTab
          boxes={visibleBoxes}
          capacity={capacity}
          onCapacityChange={regenerateBoxes}
          onChooseDriver={chooseBoxDriver}
          onMoveLastShipmentToNextBox={moveLastShipmentToNextBox}
          onSendBox={sendBox}
        />
      )}

      {activeTab === 'driverUpdates' && (
        <DriverUpdatesTab
          updates={updates}
          onApproveLowRisk={approveLowRiskUpdates}
          onDecideUpdate={decideUpdate}
        />
      )}
    </div>
  );
...
function PickupRequestsTab({
  ...
  onDispatch: (requestId: string, driverName: string, shipmentIds: string[]) => void;
}) {
  const [selectedShipments, setSelectedShipments] = useState<Record<string, string[]>>(() => (
    Object.fromEntries(pickupRequestsSeed.map((request) => [request.id, request.shipments]))
  ));
  const [selectedDrivers, setSelectedDrivers] = useState<Record<string, string>>(() => (
    Object.fromEntries(pickupRequestsSeed.map((request) => [request.id, request.drivers[0]]))
  ));
  ...
  const toggleShipment = (request: PickupRequest, shipmentId: string) => {
    setSelectedShipments((current) => {
      ...
      const next = selected.includes(shipmentId)
        ? selected.filter((id) => id !== shipmentId)
        : [...selected, shipmentId];
      return { ...current, [request.id]: next };
    });
  };

  const selectAll = (request: PickupRequest) => {
    setSelectedShipments((current) => ({ ...current, [request.id]: request.shipments }));
  };

  if (requests.length === 0) {
    return <EmptyState title="لا توجد طلبات استلام" text="كل طلبات التجار الحالية تم إرسالها للمناديب." />;
  }
            ...
            <div className="ops-row-main">
              <div>
                <div className="ops-title-line">
                  ...
                  <span className={`ops-badge ${request.priority === 'high' ? 'danger' : 'info'}`}>
                    {request.priority === 'high' ? 'عاجل' : 'طبيعي'}
                  </span>
                </div>
                <p className="ops-meta">{request.zone} - {request.address} - <span dir="ltr">{request.phone}</span></p>
              </div>
              <div className="ops-row-number">
                <strong>{request.shipments.length}</strong>
                <span>بوليصة</span>
              </div>
            </div>
            ...
            <div className="ops-row-actions">
              <label className="driver-select-field">
                ...
                <select
                  ...
                  value={driverName}
                  onChange={(event) => setSelectedDrivers((current) => ({ ...current, [request.id]: event.target.value }))}
                >
              ...
              <button className="outline-btn" onClick={() => selectAll(request)}>
                <ClipboardCheck size={15} />
                تحديد الكل
              </button>
              <button className="btn-primary" onClick={() => onDispatch(request.id, driverName, selected)}>
                <UserCheck size={15} />
                إرسال المحدد ({selected.length})
              </button>
            </div>
          </article>
        );
...
function PickupReviewTab({
  ...
  reviewLines: PickupReviewLine[];
  scanValue: string;
  onApproveReviewed: () => void;
  onMarkReviewLine: (shipmentId: string, status: PickupLineStatus) => void;
  onScan: () => void;
  ...
  return (
    <section className="ops-section">
      <div className="scanner-panel glass-card">
        ...
        <button className="outline-btn" onClick={onApproveReviewed}>اعتماد المعلم ({verifiedCount})</button>
      </div>

      {reviewLines.length === 0 ? (
        <EmptyState title="لا توجد شحنات للتأكيد" text="أي باكدج يتم إرساله من طلبات الاستلام سيظهر هنا للمراجعة." />
      ) : (
              ...
              <tbody>
                {reviewLines.map((line) => (
                  <tr key={line.id} className="table-row">
                    <td className="tracking-num">{line.id}</td>
                    <td>{line.merchantName}</td>
                    <td>{line.driverName}</td>
                    <td className="amount">{formatCurrency(line.codAmount)}</td>
                    <td><span className={`ops-badge ${pickupLineTone[line.status]}`}>{pickupLineLabels[line.status]}</span></td>
                    <td>
                      <div className="row-actions">
                        <button className="btn-icon sm" title="تمت المراجعة" onClick={() => onMarkReviewLine(line.id, 'verified')}><CheckCircle2 size={14} /></button>
                        <button className="btn-icon sm" title="ناقص" onClick={() => onMarkReviewLine(line.id, 'missing')}><XCircle size={14} /></button>
                        <button className="btn-icon sm" title="مرتجع" onClick={() => onMarkReviewLine(line.id, 'returned')}><PackageCheck size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
...
function AssignmentTab({
  ...
  onCapacityChange,
  onChooseDriver,
  onMoveLastShipmentToNextBox,
  onSendBox,
}: {
  boxes: Array<DeliveryBox & { shipments: DeliveryShipment[] }>;
  capacity: number;
  onCapacityChange: (capacity: number) => void;
  onChooseDriver: (boxId: string, driverId: string) => void;
  onMoveLastShipmentToNextBox: (boxId: string) => void;
  onSendBox: (boxId: string) => void;
}) {
  return (
    <section className="ops-section">
      <div className="ops-toolbar glass-card">
        <div>
          <strong>قدرة المندوب</strong>
          <span>الشحنات تتقسم لمجموعات حسب الزون والأولوية.</span>
        </div>
        <div className="capacity-control">
          {[3, 5, 8, 10].map((value) => (
            <button key={value} className={`capacity-chip ${capacity === value ? 'active' : ''}`} onClick={() => onCapacityChange(value)}>
              {value}
            </button>
          ))}
        </div>
      ...
      <div className="assignment-layout">
        <aside className="driver-candidates glass-card">
          ...
          {drivers.map((driver) => (
            <div key={driver.id} className="driver-candidate">
              <div>
                <strong>{driver.name}</strong>
                <p>{driver.zone} - حمولة {driver.activeLoad}/{driver.capacity}</p>
              </div>
              <span className="score-pill">{driver.score}%</span>
            </div>
          ))}
        </aside>
            ...
            return (
              <article key={box.id} className={`delivery-box glass-card ${box.sent ? 'assigned' : ''}`}>
                <div className="delivery-box-header">
                  <div>
                    <p className="ops-title">{box.id.replace('BOX', 'مجموعة')}</p>
                    <p className="ops-meta">{box.zone} - {box.shipmentIds.length} شحنات - {formatCurrency(totalCod)}</p>
                  </div>
                  <span className={`ops-badge ${box.sent ? 'success' : 'warning'}`}>{box.sent ? 'تم الإرسال' : 'جاهز'}</span>
                </div>

                <div className="delivery-box-list">
                  {box.shipments.map((shipment) => (
                    <div key={shipment.id} className="delivery-box-row">
                      <span className="tracking-num">{shipment.id}</span>
                      <span>{shipment.addressHint}</span>
                ...
                <div className="ops-row-actions">
                  <label className="driver-select-field">
                    ...
                    <select
                      className="ops-select"
                      value={box.selectedDriverId ?? ''}
                      onChange={(event) => onChooseDriver(box.id, event.target.value)}
                      disabled={box.sent}
                    >
                  ...
                  <button className="outline-btn" onClick={() => onMoveLastShipmentToNextBox(box.id)} disabled={box.sent}>
                    نقل آخر بوليصة
                  </button>
                  <button className="btn-primary" onClick={() => onSendBox(box.id)} disabled={box.sent || !selectedDriver}>
                    <Send size={15} />
                    إسناد الشحنات
                  </button>
                </div>
...
function DriverUpdatesTab({
  ...
  onDecideUpdate: (updateId: string, decision: DriverDecision) => void;
}) {
  ...
  return (
    <section className="ops-section">
      <div className="ops-toolbar glass-card">
        <div>
          <strong>{pendingCount} تحديث ينتظر قرار</strong>
          <span>اعتمد ما يظهر للتاجر، أو غير الحالة من الشركة، أو راجع المندوب.</span>
        </div>
        <button className="btn-primary" onClick={onApproveLowRisk}>
          <CheckCircle2 size={15} />
          اعتماد منخفض المخاطر
        </button>
      </div>

      <div className="updates-grid">
        {updates.map((update) => (
          <article key={update.id} className={`update-card glass-card ${update.decision !== 'pending' ? 'resolved' : ''}`}>
            <div className="pickup-card-top">
              <div>
                <p className="ops-title">{update.shipmentId} - {update.customerName}</p>
                <p className="ops-meta">{update.driverName} - {update.merchantName}</p>
              </div>
              <span className={`ops-badge ${riskTone[update.risk]}`}>{riskLabels[update.risk]}</span>
            </div>

            <div className="status-change-line">
              <span>{update.fromStatus}</span>
              <Send size={14} />
              <strong>{update.toStatus}</strong>
            </div>
            <p className="ops-meta">{update.note}</p>

            {update.decision === 'pending' ? (
              <div className="ops-actions">
                <button className="btn-primary" onClick={() => onDecideUpdate(update.id, 'approved')}>
                  <CheckCircle2 size={15} />
                  اعتمده
                </button>
                <button className="outline-btn" onClick={() => onDecideUpdate(update.id, 'changed')}>غير الحالة</button>
                <button className="outline-btn" onClick={() => onDecideUpdate(update.id, 'review')}>راجع المندوب</button>
              </div>
            ) : (
              <p className="decision-note">{decisionLabels[update.decision]}</p>
            )}
          </article>
        ))}
      </div>
...
function MetricPill({ label, value, tone, onClick }: { label: string; value: number; tone: string; onClick: () => void }) {
  return (
    <button className={`metric-pill ${tone}`} onClick={onClick}>
      <span>{label}</span>
      <strong>{value}</strong>
    </button>
  );
...
function EmptyState({ title, text }: { title: string; text: string }) {
  return (
    <div className="empty-ops glass-card">
      <Truck size={22} />
      <strong>{title}</strong>
      <span>{text}</span>
    </div>
...
function buildBoxes(shipments: DeliveryShipment[], capacity: number): DeliveryBox[] {
  const groupedByZone = shipments
    .slice()
    .sort((a, b) => a.zone.localeCompare(b.zone, 'ar') || b.priority - a.priority)
    .reduce<Record<string, DeliveryShipment[]>>((groups, shipment) => {
  ...
  return Object.entries(groupedByZone).flatMap(([zone, zoneShipments]) => {
    ...
    for (let index = 0; index < zoneShipments.length; index += capacity) {
      ...
      result.push({
        id: `BOX-${zone.slice(0, 2)}-${String(result.length + 1).padStart(2, '0')}`,
        zone,
        shipmentIds,
        selectedDriverId: suggestedDriver?.id,
      });
    }
...
function suggestDriver(box: Pick<DeliveryBox, 'zone' | 'shipmentIds'>): DriverCandidate | undefined {
  return drivers
    .filter((driver) => driver.zone === box.zone && driver.capacity - driver.activeLoad >= box.shipmentIds.length)
    .sort((a, b) => b.score - a.score)[0];
}

const pickupLineLabels: Record<PickupLineStatus, string> = {
...
const pickupLineTone: Record<PickupLineStatus, string> = {
  waiting: 'warning',
  verified: 'success',
  missing: 'danger',
  returned: 'danger',
...
const decisionLabels: Record<DriverDecision, string> = {
  pending: 'بانتظار القرار',
  approved: 'تم اعتماده وسيظهر للتاجر',
  review: 'تم تحويله للمراجعة مع المندوب',
  changed: 'تم تغيير الحالة من الشركة',
};
