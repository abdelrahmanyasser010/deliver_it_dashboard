...
const tabs: Array<{ id: OpsTab; label: string; icon: typeof Route }> = [
  ...
  { id: 'deliveryAssignments', label: 'توزيع الشحنات', icon: Truck },
...
export function OperationsCenterPage() {
  ...
  const suggestedDrivers = useMemo(() => drivers.filter((driver) => driver.status === 'active' && driver.availability !== 'offline').sort((a, b) => (b.capacity - b.activeLoad) - (a.capacity - a.activeLoad)), [drivers]);
  ...
  return <div className="operations-center">
    <section className="ops-command-bar glass-card">
      ...
      <div className="ops-command-metrics">
        ...
        <button className="metric-pill info" onClick={() => selectTab('driverUpdates')}>اعتمادات <strong>{number(counts.driverUpdates)}</strong></button>
    ...
    {activeTab === 'pickupRequests' && <section className="ops-section compact-flow">{pickupTasks.filter((task) => matches([task.id, task.merchantName, task.driverName])).map((task) => {
    ...
    })}{pickupTasks.length === 0 && <EmptyState title="لا توجد مهام استلام" description="كل طلبات الاستلام الحالية تمت معالجتها."/>}</section>}

    {activeTab === 'officeReview' && <section className="ops-section"><div className="ops-next-step glass-card"><div><strong>الاستلام بالباركود هو مصدر تأكيد الوصول</strong><span>أنشئ دفعة مرتبطة بطلب الاستلام؛ عند إغلاقها ستتحول الشحنات إلى «وصلت المكتب» وتظهر للتوزيع.</span></div><button className="btn-primary" onClick={() => navigate('/barcode')}><Barcode size={16}/> فتح شاشة الباركود</button></div>{officeShipments.filter((shipment) => matches([shipment.id, shipment.merchantName, shipment.customerName])).slice(0, 12).map((shipment) => <ShipmentTaskRow key={shipment.id} shipment={shipment} action={<button className="outline-btn" onClick={() => navigate(`/shipments?shipment=${shipment.id}`)}>فتح الشحنة</button>}/>)}</section>}
