...
export function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const statuses = [
      ...
      { label: 'تم التوصيل كاملًا', count: totals.delivered, query: 'delivered' },
      { label: 'تم التسليم جزئيًا', count: totals.partialDelivered, query: 'partiallyDelivered' },
    ...
    const governorates = [...new Set(current.map((item) => item.governorate))].map((governorate) => {
      ...
      return { governorate, total: items.length, delivered: deliveredItems.length, inTransit: items.filter((item) => item.status === 'inTransit').length, returned: items.filter((item) => item.status === 'returned').length, delayed: items.filter((item) => item.expectedDeliveryAt && new Date(item.expectedDeliveryAt).getTime() < now && !['delivered', 'partiallyDelivered', 'returned'].includes(item.status)).length, avgDeliveryHours: deliveredItems.length ? Math.round(deliveredItems.reduce((sum, item) => sum + Math.max(1, (new Date(item.statusChangedAt).getTime() - new Date(item.createdAt).getTime()) / 3600000), 0) / deliveredItems.length) : 0 };
    });
    const delays = current.filter((item) => item.expectedDeliveryAt && new Date(item.expectedDeliveryAt).getTime() < now && !['delivered', 'partiallyDelivered', 'returned'].includes(item.status)).map((item) => ({ shipment: item, lateByHours: Math.max(1, Math.round((now - new Date(item.expectedDeliveryAt!).getTime()) / 3600000)) })).sort((a, b) => b.lateByHours - a.lateByHours);
  ...
  const exportActiveReport = () => {
    ...
    setActionMessage('تم تجهيز ملف Excel حقيقي مطابق للفترة والفلاتر الحالية.');
  };
  ...
  return <div className="reports-page">
    ...
    {activeTab === 'operations' && <div className="report-grid-2"><section className="glass-card"><div className="report-section-title"><h3>مسار حالات الشحنات</h3><span className="report-muted" title="الشحنات المسلمة ÷ الشحنات التي وصلت لمحاولة توصيل">نسبة نجاح المحاولات {pct(report.successRate)} ({report.successRate - report.previousSuccessRate >= 0 ? '+' : ''}{pct(report.successRate - report.previousSuccessRate)})</span></div><div className="funnel-list">{report.statuses.map((step) => <button key={step.label} className="funnel-row" onClick={() => navigate(withDates(`/shipments?status=${step.query}`))}><ProgressRow label={step.label} value={step.count} max={Math.max(1, report.current.length)} suffix="شحنة"/></button>)}</div></section><section className="glass-card"><div className="report-section-title"><h3>تعريف المؤشر</h3></div><p className="report-muted">نسبة النجاح تشمل التسليم الكامل والجزئي من الشحنات التي وصلت إلى نتيجة محاولة توصيل. ويظهر التسليم الجزئي كمؤشر مستقل لأنه قد ينتج عنه إعادة محاولة أو مرتجع.</p><div className="report-kpi-grid one-column"><Kpi label="وصل مكتب الشحن" value={fmt(report.statuses[1].count)} icon={<PackageCheck size={20}/>} gradient="linear-gradient(135deg,#0EA5E9,#0284C7)"/><Kpi label="تسليم جزئي" value={fmt(report.totals.partialDelivered)} icon={<PackageCheck size={20}/>} gradient="linear-gradient(135deg,#14B8A6,#0F766E)" onClick={() => navigate(withDates('/shipments?status=partiallyDelivered'))}/><Kpi label="مع المندوب أو في الطريق" value={fmt(report.statuses[2].count + report.statuses[3].count)} icon={<Truck size={20}/>} gradient="linear-gradient(135deg,#8B5CF6,#6D28D9)"/></div></section></div>}
    ...
    {activeTab === 'governorates' && <section className="glass-card"><div className="report-section-title"><h3>أداء المحافظات</h3></div><div className="table-wrapper"><table className="data-table"><thead><tr><th>المحافظة</th><th>الإجمالي</th><th>تم التسليم</th><th>في الطريق</th><th>مرتجع</th><th>متأخر</th><th>متوسط الساعات</th><th>إجراء</th></tr></thead><tbody>{report.governorates.map((item) => <tr key={item.governorate}><td><strong>{item.governorate}</strong></td><td>{fmt(item.total)}</td><td>{fmt(item.delivered)}</td><td>{fmt(item.inTransit)}</td><td>{fmt(item.returned)}</td><td>{fmt(item.delayed)}</td><td>{fmt(item.avgDeliveryHours)}</td><td><button className="outline-btn" onClick={() => setGovernorateDetails(item.governorate)}><Eye size={14}/> التفاصيل</button></td></tr>)}</tbody></table></div></section>}
