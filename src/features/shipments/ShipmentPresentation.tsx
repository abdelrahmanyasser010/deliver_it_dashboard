...
export function ShipmentRow({ shipment, checked, onToggle, onOpen, onPrint, now, visibleColumns }: { shipment: Shipment; checked: boolean; onToggle: () => void; onOpen: () => void; onPrint: () => void; now: number; visibleColumns: ShipmentColumn[] }) {
  ...
  const isDelayed = shipment.expectedDeliveryAt && new Date(shipment.expectedDeliveryAt).getTime() < now && !['delivered', 'returned'].includes(shipment.status);
...
export function ShipmentDrawer({ shipment, attempts, activeAction, drivers, onClose, onAction, onCancelAction, onSubmitAction, onPrint }: { shipment: Shipment; attempts: string[]; activeAction: ShipmentAction | null; drivers: Array<{ id: string; name: string }>; onClose: () => void; onAction: (action: ShipmentAction) => void; onCancelAction: () => void; onSubmitAction: (payload: Record<string, string>) => void; onPrint: () => void }) {
...
function Timeline({ shipment, attempts }: { shipment: Shipment; attempts: string[] }) {
  const items = [
    ...
    { title: 'وصلت مكتب الشحن', detail: 'تم استلامها ومراجعتها داخليًا', done: ['receivedAtOffice', 'deliveredToDriver', 'inTransit', 'delivered', 'postponed', 'failedToDeliver', 'returned'].includes(shipment.status) },
